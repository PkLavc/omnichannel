...
def main():
    ...
    parser.add_argument("--prefix", required=True, help="Prefixo neutro do tenant; mantenha o nome comercial somente na pasta privada.")
    ...
    for source in sorted(args.input_dir.glob("role-corpus/*.jsonl")):
        ...
        with target.open("w", encoding="utf-8") as handle:
            handle.write(f"# RAG do agente {labels.get(role, role)}\n\n")
    ...
    print(f"generated={len(list(args.output_dir.glob(f'{args.prefix}-*.md')))}")
